...
To make sense at how an NN train and learns, you first need a solid understand of what a derivative is.
...
This function is easy to understand and derivate analytic, the derivate is $\frac{df(x)}{dx} = 6x - 4$. Plotting both function and his derivate, we get the graphic below.
```{python}
#| code-fold: true
#| code-summary: "Show Python code"
import numpy as np
...
class Value:
    def __init__(self, data, _children=(), _op="", label=""):
        ...
        self._prev = set(_children) # Don't worry about this for know, we only use set for a little bit better performance
...
def draw_dot(root):
    # Removi o 'size' forçado. Adicionei ranksep e nodesep para dar um respiro visual.
    dot = Digraph(format='svg', graph_attr={'rankdir': 'LR', 'ranksep': '0.5', 'nodesep': '0.5'}) 
    
    nodes, edges = trace(root)
    for n in nodes:
        uid = str(id(n))
        # shape='record' é perfeito para mostrar dados e gradientes juntos
        dot.node(name=uid, label="{%s| data %.4f | grad %.4f }" % (n.label, n.data, n.grad), shape='record')
        if n._op:
            # Cria o nó da operação (menor)
...
$$
Bascially, it´s saying that the gradient of $e$ in relation with $L$ it´s just the gradient of $d$ in relation with $L$ time $e$ in relation with $d$. This mean
that we only have to calc the local gradient $\frac{\partial d}{\partial e}$ because we already have calc the $\frac{\partial L}{\partial d}$ and its save in d.grad. The same with $c$ it´s true.
...
To do this automatically, you probably noticed that we need to start from the last node and work backwards (from the last to the first). The reverse order is necessary because for calculate the gradient of any node, we need the local gradient of the variable, and the gradient of the father, the unique exception is the last node, because its gradient always will be 1.
he way we will implement this is by making each node responsible for calculating and passing the gradients down to its children. Let's see how this works for the multiplication (mul) function::
```{python}
class Value:
    def __init__(self, data, _children=(), _op="", label=""):
        self.data = data
...
It´s worked perfectly again, know we already have a functional AutoGrad system, know, it´s just need to add more operations and we will be capable of create our propely framework of Deep Learning using our own AutoGrad. Before going to the next part, see the code bellow, its the same that we create together, but
...
Work, it's probability possible to improve this changing the activation function by an ReLU. But for our propose, it's good enough. Lets just see the values coming from our model
