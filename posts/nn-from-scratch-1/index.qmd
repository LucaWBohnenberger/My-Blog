...
class Value:
    ...
    def __pow__(self, other):
        ...
        def _backward():
            # Regra da cadeia para potência: n * x^(n-1) * grad_out
            self.grad += (other * self.data**(other-1)) * out.grad
        out._backward = _backward
    ...
    def __radd__(self, other): # Para permitir sum([Value, Value]) onde começa com 0 + Value
        return self + other
...
def f(x,y,z):
    return 0.4*x**2 + 0.7*y*x - z + random.gauss(0, 1)
...
Work, but not very well, it's possible to improve this changing the activation function by an ReLU. But for our propose, it's good enough. Lets just see the values comming from our model
